...
class EmailAnalzerEngine:
    ...
    def _smart_prepare(self, data):
        ...
        for block in b64_blocks:
            try:
                ...
                if missing_padding: pure_block += '=' * (4 - missing_padding)
    ...
    def get_full_headers(self): 
        ...
        for key in target_keys:
            ...
            if match:
                ...
                clean_value = re.sub(REGEX["prepare"]["multi_space"], ' ', raw_value).strip()
                headers[key] = clean_value
            else:
                headers[key] = "N/A"
        
        if headers["From"] == "N/A" or "<" not in headers["From"]:
            backup_match = re.search(REGEX["headers"]["from_fallback"], self.raw_data, re.S | re.I)
            if backup_match:
                headers["From"] = f"{headers.get('From', '')} <{backup_match.group(1)}>".replace("N/A ", "")
    ...
    def check_authentication(self): 
        ...
        if "spf=pass" in low: auth["SPF"] = "PASS"
        elif "spf=fail" in low: auth["SPF"] = "FAIL"
        if "dkim=pass" in low: auth["DKIM"] = "PASS"
        elif "dkim=fail" in low: auth["DKIM"] = "FAIL"
        if "dmarc=pass" in low: auth["DMARC"] = "PASS"
        elif "dmarc=fail" in low: auth["DMARC"] = "FAIL"
        return auth
    ...
    def get_attachments_info(self):
        files = re.findall(REGEX["attachments"]["filename"], self.raw_data)
        attachments = []
        ...
        for f in set(files):
            ...
            raw_b64 = ""
            if pattern_b64:
                match = re.search(pattern_b64.replace("{filename}", re.escape(f)), self.raw_data, re.S)
                if match:
...
class SplashScreen(ctk.CTk):
    def __init__(self):
        ...
        self.brand_label = ctk.CTkLabel(self.frame, text="PhishyEmailAnalyzer", 
                                        font=("Impact", 40), text_color=BLUE_MAIN)
        self.brand_label.pack()

        self.tagline = ctk.CTkLabel(self.frame, text="ADVANCED EMAIL THREAT INTELLIGENCE", 
                                    font=("Consolas", 12), text_color="#64748b")
        self.tagline.pack(pady=(0, 30))

        self.progress = ctk.CTkProgressBar(self.frame, width=400, height=12, 
                                           progress_color=BLUE_MAIN, fg_color="#1e293b")
        self.progress.pack(pady=20)
        self.progress.set(0)

        self.status_label = ctk.CTkLabel(self.frame, text="Loading Engine", 
                                         font=("Consolas", 10), text_color=BLUE_MAIN)
...
class App(ctk.CTk):
    def __init__(self):
        ...
        icon_path = resource_path("icon.ico")
        self.iconbitmap(icon_path)
        self.title("PhishyEmailAnalyzer")
        ...
        self.top_bar = ctk.CTkFrame(self.content_area, height=70, fg_color=CARD_BG, corner_radius=0)
        self.top_bar.pack(side="top", fill="x")
        self.status_title = ctk.CTkLabel(self.top_bar, text="DASHBOARD", font=("Segoe UI", 16, "bold"), text_color=BLUE_MAIN)
        ...
        self.nav_buttons = {}
        self.pages = {name: ctk.CTkFrame(self.main_container, fg_color="transparent") for name in ["Input", "Results"]}
    ...
    def create_nav_menu(self):
        for icon, page_id in [("📥", "Input"), ("📊", "Results")]:
            btn = ctk.CTkButton(self.sidebar, text=icon, width=65, height=65, corner_radius=18, fg_color="transparent", font=("Segoe UI", 26), hover_color=BLUE_DARK, command=lambda p=page_id: self.show_page(p))
            btn.pack(pady=20, padx=12); self.nav_buttons[page_id] = btn
    ...
    def setup_input_page(self):
        ...
        card = ctk.CTkFrame(page, fg_color=CARD_BG, corner_radius=35, border_width=1, border_color="#1e293b")
        card.place(relx=0.5, rely=0.5, anchor="center", relwidth=0.85, relheight=0.85)
        ctk.CTkLabel(card, text="EMAIL SOURCE CODE", font=("Segoe UI", 26, "bold"), text_color="#f8fafc").pack(pady=(45, 5))
        self.input_text = ctk.CTkTextbox(card, fg_color=BG_DEEP, corner_radius=20, border_width=1, border_color="#334155", font=("Consolas", 14), text_color=BLUE_MAIN)
        ...
        ctk.CTkButton(card, text="START SCAN", height=65, fg_color=BLUE_MAIN, font=("Segoe UI", 17, "bold"), command=self.run_analysis).pack(pady=45, padx=50, fill="x")

    def handle_control_shortcuts(self, event):
        if event.keycode == 86 or event.keysym.lower() == 'v':
            try: self.input_text.insert("insert", self.clipboard_get()); return "break"
            except: pass
        elif event.keycode == 67 or event.keysym.lower() == 'c':
            try: self.clipboard_clear(); self.clipboard_append(self.input_text.get("sel.first", "sel.last")); return "break"
            except: pass
        elif event.keycode == 65 or event.keysym.lower() == 'a':
            self.input_text.tag_add("sel", "1.0", "end"); return "break"
    ...
    def render_unified_results(self, engine, raw_data):
        ...
        meta_card = ctk.CTkFrame(row1, fg_color=CARD_BG, corner_radius=20, border_width=1, border_color="#1e293b")
        ...
        for key in ["Subject", "From", "To", "Date", "Reply-To", "Return-Path"]:
            val = headers.get(key)
            if val: self.create_info_row(meta_card, key, val, BLUE_MAIN)
        ctk.CTkLabel(meta_card, text="", height=10).pack() 

        auth_card = ctk.CTkFrame(row1, fg_color=CARD_BG, corner_radius=20, border_width=1, border_color="#1e293b")
        ...
        for k, v in auth.items():
            ...
            ctk.CTkLabel(row, text=k.upper(), font=("Segoe UI", 11, "bold"), text_color=BLUE_MAIN, width=80, anchor="w").pack(side="left")
            ctk.CTkLabel(row, text=status_clean, font=("Consolas", 13, "bold"), text_color=color).pack(side="left", padx=10)
            if k.upper() in ["SPF", "DKIM"]:
                dom_auth = self.extract_auth_domain(k, raw_data)
                d_lbl = ctk.CTkLabel(row, text="[Domain]", font=("Segoe UI", 10, "italic"), text_color="#64748b", cursor="hand2")
                d_lbl.pack(side="right", padx=10)
                ToolTip(d_lbl, dom_auth if dom_auth else "No domain detected", side="left")

        # --- IP SECTION ---
        ip_card = ctk.CTkFrame(scroll, fg_color=CARD_BG, corner_radius=20, border_width=1, border_color="#1e293b")
        ...
        ctk.CTkLabel(ip_row, text="LAST MTA IP:", font=("Segoe UI", 15, "bold"), text_color=BLUE_MAIN).pack(side="left")
        ctk.CTkLabel(ip_row, text=mta_ip, font=("Consolas", 18, "bold"), text_color="#ffffff").pack(side="left", padx=25)
        if mta_ip != "N/A":
            ctk.CTkButton(
                ip_row, text="AbuseIPDB", width=95, height=32, fg_color=BLUE_DARK, font=("Segoe UI", 11, "bold"),
        ...
        for key in ["From", "Return-Path", "Reply-To"]:
            ...
            if val:
                ...
                if match: target_domains.add(match.group(1).lower().strip('>'))

        if target_domains:
            self.create_section_header(scroll, "DOMAIN INTELLIGENCE", BLUE_MAIN)
            dom_card = ctk.CTkFrame(scroll, fg_color=CARD_BG, corner_radius=20, border_width=1, border_color="#1e293b")
            dom_card.pack(fill="x", padx=10, pady=5)
            for dom in target_domains:
                dr = ctk.CTkFrame(dom_card, fg_color="transparent")
                dr.pack(fill="x", padx=30, pady=15)
                ctk.CTkLabel(dr, text=dom, font=("Consolas", 14, "bold"), text_color="#ffffff").pack(side="left")
                bf = ctk.CTkFrame(dr, fg_color="transparent")
                bf.pack(side="right")
                for bt, bu in [("Check Age", LINKS["domain_intelligence"]["check_age"]),
                               ("VirusTotal", LINKS["domain_intelligence"]["virustotal"]),
                               ("Browserling", LINKS["domain_intelligence"]["browserling"])]:
                    ctk.CTkButton(
                        bf, text=bt, width=95, height=32, fg_color=BLUE_DARK,
                        font=("Segoe UI", 11, "bold"),
                        command=lambda u=bu.replace("{domain}", dom): webbrowser.open(u)
                    ).pack(side="left", padx=3)

        # --- ATTACHMENTS ---
        self.create_section_header(scroll, "ATTACHMENTS", BLUE_MAIN)
        at_container = ctk.CTkFrame(scroll, fg_color=CARD_BG, corner_radius=20, border_width=1, border_color="#1e293b")
        at_container.pack(fill="x", padx=10, pady=5)
        attachments = engine.get_attachments_info() 
        if not attachments:
            ctk.CTkLabel(at_container, text="No attachments detected.", text_color="#64748b").pack(pady=25)
        else:
            for att in attachments:
                row = ctk.CTkFrame(at_container, fg_color="transparent")
                row.pack(fill="x", pady=12, padx=30)
                ctk.CTkLabel(row, text=att['filename'], font=("Consolas", 14, "bold"), text_color="#ffffff").pack(side="left", pady=(7,7))
                btn_f = ctk.CTkFrame(row, fg_color="transparent")
                btn_f.pack(side="right")
                ctk.CTkButton(
                    btn_f, text="VirusTotal", width=95, height=32, fg_color=BLUE_DARK, font=("Segoe UI", 11, "bold"),
                ...
                ctk.CTkButton(
                    btn_f, text="Download", width=95, height=32, fg_color=BLUE_DARK, font=("Segoe UI", 11, "bold"),
                    command=lambda b=att.get('raw_b64', ''): webbrowser.open(
                        LINKS["attachments"]["cyberchef"].replace("{b64}", urllib.parse.quote(
                base64.b64encode(b.encode()).decode().rstrip("="))
                        )
                    )
                ).pack(side="left", padx=3)
        ...
        u_card = ctk.CTkFrame(scroll, fg_color=CARD_BG, corner_radius=20, border_width=1, border_color="#1e293b")
        u_card.pack(fill="x", padx=10, pady=5)
        for url_obj in engine.extract_urls():
            final_u = self.extract_final_url(url_obj['original'])
            u_row = ctk.CTkFrame(u_card, fg_color="#1e293b" if final_u != url_obj['original'] else "transparent", corner_radius=12)
            u_row.pack(fill="x", pady=5, padx=20)
            disp = self.smart_truncate(final_u, 75)
            lbl = ctk.CTkLabel(u_row, text=disp, font=("Consolas", 11), text_color="#cbd5e1", cursor="hand2")
            ...
            ctk.CTkButton(
                bf, text="VirusTotal", width=95, height=32, fg_color=BLUE_DARK, font=("Segoe UI", 11, "bold"),
                command=lambda t=final_u: webbrowser.open(
                    LINKS["urls"]["virustotal"].replace("{domain}", self.get_domain_only(t))
                )
            ).pack(side="left", padx=3)

            ctk.CTkButton(
                bf, text="Browserling", width=95, height=32, fg_color=BLUE_DARK, font=("Segoe UI", 11, "bold"),
                command=lambda t=final_u: webbrowser.open(
            ...
            ctk.CTkButton(
                bf, text="Unshorten IT", width=95, height=32, fg_color=BLUE_DARK, font=("Segoe UI", 11, "bold"),
                command=lambda t=final_u: webbrowser.open(
    ...
    def create_section_header(self, master, text, color):
        ctk.CTkLabel(master, text=text, font=("Segoe UI", 15, "bold"), text_color=color).pack(pady=(25, 12), padx=30, anchor="w")

    def create_info_row(self, master, key, value, k_color):
        row = ctk.CTkFrame(master, fg_color="transparent")
        row.pack(fill="x", pady=5, padx=30)
        ctk.CTkLabel(row, text=key.upper(), font=("Segoe UI", 11, "bold"), text_color=k_color, width=110, anchor="w").pack(side="left")
        ctk.CTkLabel(row, text=value, font=("Consolas", 13), text_color="#cbd5e1", wraplength=450, justify="left").pack(side="left", padx=15)
    ...
    def get_domain_only(self, url):
        try: return urlparse(url).netloc if urlparse(url).netloc else url
        except: return url
